...
| docker volume                   | ls 查看所有挂载列表                                                                                      | docker volume inspect 挂载名 查看具名挂载信息                                             |
| docker save                     | -o                                                                                               |                                                                                |
| docker load                     | -i                                                                                               |                                                                                |
|                                 |                                                                                                  |                                                                                |

## 6.镜像加载原理
1. Docker镜像由一层一层文件系统组成，这种层级的文件系统叫UnionFS联合文件系统。
2. UnionFS文件系统是Docker镜像的基础，镜像可以通过分层来继承，基于基础镜像，制作各种具体的应用镜像。
3. bootfs（引导加载）主要包含bootloader和kernel，bootloader主要是引导加载kernel。Docker镜像最底层是bootfs，包含boot加载器和内核，当boot加载完成后，内核就在内存中了，此时内存使用权由bootfs交给内核，系统也会卸载bootfs。
4. rootfs，在bootfs之上，包含典型Linux系统中/dev、/proc、/bin、/etc等标准目录和文件。rootfs就是各种不同的操作系统发行版，如Ubuntu、Centos。
5. 引导（Booting）是宿主机的任务。在你的服务器或电脑开机时，已经完成了从`bootfs`加载内核、启动`init`系统等所有引导步骤。宿主机内核已经像一个完全启动好的大脑在运行了。`bootfs`只在**宿主机**层面存在和使用了一次。容器**直接使用**这个已经启动好的内核，自己不需要，也看不到`bootfs`。Docker容器没有，也不需要引导过程。

## 7.镜像分层原理

![](../../attachments/tech-20251230130041.png)
>Docker镜像是只读的，容器启动时，一个新的可写层被加载到镜像的顶部。
...
# 实现
1. 容器间有个数据分享的技术，Docker容器中产生的数据，同步到本地，也就是所谓的卷技术。
# 指定路径挂载
docker run -it  -v /Users/xx/Desktop/study/docker/test:/home centos:7 sh
# 容器删除后，本地文件不会丢失
# 具名挂载（卷名:容器内路径）
docker run -d --name nginx_demo -p 3344:80 -v nginx_demo:/etc/nginx nginx
# 查看具名挂载信息（没有指定目录情况下都在：/var/lib/docker/volumes/）
docker volume inspect nginx_demo
# 匿名挂载（不写主机路径）
docker run -d --name nginx_demo -p 3344:80 -v /etc/nginx nginx
# 查看所有卷情况
docker volume ls
# 设置容器权限（ro说明这个路径只能通过宿主机操作，容器内部无法操作）
docker run -d --name nginx_demo -p 3344:80 -v nginx_demo:/etc/nginx:ro nginx
# 容器之间数据卷同步（-volumes -from）
docker run -it --name es01 es:0.0.1
docker run -it --name es02 --volumes-from es01 es:0.0.1
docker run -d -p 3310:3306 -v /etc/mysql/conf.d -v /var/lib/mysql -e MYSQL_ROOT_PASSWORD=123456 --name mysql_demo01 mysql
docker run -d -p 3311:3306 -v /etc/mysql/conf.d -v /var/lib/mysql -e MYSQL_ROOT_PASSWORD=123456 --name mysql_demo02 --volumes-from mysql_demo01 mysql
```
...
```shell
# 运行
docker run -d -p 3310:3306 -v /home/mysql/conf:/etc/mysql/conf.d -v /home/mysql/data:/var/lib/mysql -e MYSQL_ROOT_PASSWORD=123456 --name mysql_demo mysql
```
## 11.发布docker hub
```shell
# 登录
